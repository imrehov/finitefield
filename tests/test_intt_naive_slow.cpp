...
TEST_CASE("inverse NTT reverses forward NTT vec") {
    ...
    auto transformed = ntt_naive_slow<4, 17>(a);
    auto recovered = intt_naive_slow<4, 17>(transformed);

    CHECK(recovered == a);
}

TEST_CASE("inverse NTT works for known transformed vector vec") {
    std::vector<mod_t<17>> transformed{
        mod_t<17>{10},
        mod_t<17>{7},
        mod_t<17>{15},
        mod_t<17>{6}
